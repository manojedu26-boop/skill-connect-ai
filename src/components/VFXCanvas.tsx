...
const BlobShader = {
  ...
  vertexShader: `
    ...
    void main() {
      vUv = uv;
      float noise = snoise(vec3(position.xy * 0.5, uTime * 0.5));
      float mouseDist = distance(position.xy, uMouse * 2.0);
      float mouseImp = 1.0 / (1.0 + mouseDist * 2.0);
      
      vDistortion = noise + mouseImp * 0.5;
      vec3 pos = position + normal * vDistortion * 0.5;
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      gl_FragColor = vec4(color, 0.4 + vDistortion * 0.3);
    }
  `
};
...
const Blob = () => {
  ...
  const uniforms = useMemo(() => BlobShader.uniforms, []);
